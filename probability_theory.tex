...
とする離散確率変数$X$の分布を離散一様分布という．
 ...
 \begin{align}
  M_X\left(t\right)&=G_X\left(e^t\right)\nonumber\\
  &=\sum_{x=a}^b\frac{e^{tx}}{n}\\
  M_X^{\left(1\right)}\left(t\right)&=\frac{d}{dt}\sum_{x=a}^b\frac{e^{tx}}{n}\nonumber\\
  &=\sum_{x=a}^b\frac{d}{dt}\frac{e^{tx}}{n}\nonumber\\
  &=\sum_{x=a}^b\frac{1}{n}\frac{de^{tx}}{dt}\nonumber\\
  &=\sum_{x=a}^b\frac{xe^{tx}}{n}
 \end{align}
 \subsection{特性関数}
 \begin{align}
  \phi_X\left(t\right)&=M_X\left(it\right)\nonumber\\
  &=\sum_{x=a}^b\frac{e^{itx}}{n}
 \end{align}
 \subsection{キュムラント母関数}
 \begin{align}
  K_X\left(t\right)&=\log M_X\left(t\right)\nonumber\\
  &=\log\sum_{x=a}^b\frac{e^{tx}}{n}
 \end{align}
 \subsection{平均}
...
よって成り立つ．
 ...
 \begin{align}
  ...
  &=\frac{1}{n}\frac{n\left(a+b\right)}{2}\nonumber\\
  &=\frac{a+b}{2}
 \end{align}
 \subsection{分散}
