...
互いに独立な$N$個の確率変数$X_1,\cdots,X_N$の和$X=\sum_{n=1}^NX_n$の積率母関数$M_X$は，
 \begin{align}
  M_X\left(t\right)&=\prod_{n=1}^NM_{X_n}\left(t\right)
 \end{align}
である．
 \subsubsection{証明}
...
である．
 ...
 \begin{align}
  ...
  &=G_{aX+b}\left(e^{it}\right)\nonumber\\
  &=E\left(e^{it\left(aX+b\right)}\right)\nonumber\\
  &=E\left(e^{iatX+ibt}\right)\nonumber\\
  &=E\left(e^{iatX}e^{ibt}\right)\nonumber\\
  &=e^{ibt}E\left(e^{iatX}\right)\nonumber\\
  &=e^{ibt}G_X\left(e^{iat}\right)\nonumber\\
...
式(\ref{ProductAverage})を用いて証明する．
 \begin{align}
  \phi_X\left(t\right)&=M_X\left(it\right)\nonumber\\
  &=G_X\left(\exp\left(e^{it}\right)\right)\nonumber\\
  &=E\left(\exp\left(itX\right)\right)\nonumber\\
...
ここで，$X_0,\cdots,X_{N-1}$は互いに独立だから式(\ref{ProductAverage})により，
 \begin{align}
  ...
  &=\prod_{n=0}^{N-1}G_{X_n}\left(e^{it}\right)\nonumber\\
  &=\prod_{n=0}^{N-1}M_{X_n}\left(it\right)\nonumber\\
...
$N$個の中から$n$個選ぶ場合の数は$\binom{N}{n}$で，選ばれた$n$個が全て$1$になる確率は$p_n$，残りの$N-n$個が全て$0$になる確率は$\left(1-p\right)^{N-n}$であるから，$X=n$となる確率は，
 \begin{align}
...
である．
 ...
 \begin{align}
  G_X\left(t\right)&=E\left(t^X\right)\nonumber\\
  &=\sum_{n=0}^Np\left(n\right)t^n\nonumber\\
  &=\sum_{n=0}^N\binom{N}{n}p^n\left(1-p\right)^{N-n}t^n\nonumber\\
  &=\sum_{n=0}^N\binom{N}{n}\left(pt\right)^n\left(1-p\right)^{N-n}\nonumber\\
  &=\left(pt+1-p\right)^N
 \end{align}
 \subsection{積率母関数}
 \begin{align}
  M_X\left(t\right)&=E\left(e^{tX}\right)\nonumber\\
  &=\sum_{n=0}^Np\left(n\right)e^{tn}\nonumber\\
  &=\sum_{n=0}^N\binom{N}{n}p^n\left(1-p\right)^{N-n}e^{tn}\nonumber\\
  &=\sum_{n=0}^N\binom{N}{n}\left(pe^t\right)^n\left(1-p\right)^{N-n}\nonumber\\
