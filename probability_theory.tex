...
ただし，Bellの多項式が部分集合同士の順番を考えていないのに対し，多項式の$k$乗の展開は，$k$個の順序付けられた多項式因子それぞれからひとつの項を因子として選択することによって展開後のひとつの項を計算することを考えると，ひとつの項を計算する際に選ばれた$k$個の因子の可能な全ての順番の数$k!$を掛ける必要があるから，
 \begin{align}
  g\left(x\right)&=\sum_{m=0}^\infty k!B_{m,k}\left(\frac{f^{\left(0\right)}\left(0\right)}{0!}x^0,\cdots,\frac{f^{\left(n\right)}\left(0\right)}{n!}x^n,\cdots,\frac{f^{\left(m-k+1\right)}\left(0\right)}{\left(m-k+1\right)!}x^{m-k+1}\right)\nonumber\\
  &=k!\sum_{m=0}^\infty B_{m,k}\left(\frac{f^{\left(0\right)}\left(0\right)}{0!},\cdots,\frac{f^{\left(n\right)}\left(0\right)}{n!},\cdots,\frac{f^{\left(m-k+1\right)}\left(0\right)}{\left(m-k+1\right)!}\right)x^m
 \end{align}
である．
