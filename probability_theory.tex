...
確率変数$X$に対して，
 ...
 \end{align}
を$X$の尖度といい，分散よりも外れ値への反応をより敏感にした指標であり，分布の裾の重さを表す．
尖度という名称から分布の平均周りの尖り具合を表していると誤解されやすいが，尖り具合を表してはいない．
 \section{エントロピー}
確率変数$X$に対して，
 \begin{align}
...
標本空間を
 \begin{align}
  S\left(X\right)&=\{a,\cdots,b\}\subset\mathbb{Z}\\
  |S\left(X\right)|&=b-a+1=n
 \end{align}
とし，確率質量関数を
 \begin{align}
  \forall x\in S\left(X\right),p\left(x\right)=\frac{1}{n}
 \end{align}
とする離散確率変数$X$の分布を離散一様分布という．
 \subsection{確率母関数}
 \begin{align}
  G_X\left(t\right)&=E\left(t^X\right)\nonumber\\
  &=\sum_{x\in S\left(X\right)}t^xp\left(x\right)\nonumber\\
  &=\sum_{x=a}^b\frac{t^x}{n}\nonumber\\
  &=\frac{1}{n}\sum_{x=a}^bt^x
 \end{align}
 \subsection{積率母関数}
 \begin{align}
  M_X\left(t\right)&=G_X\left(e^t\right)\nonumber\\
  &=\frac{1}{n}\sum_{x=a}^be^{tx}\\
  M_X^{\left(n\right)}\left(t\right)&=\frac{d^n}{dt^n}\frac{1}{n}\sum_{x=a}^be^{tx}\nonumber\\
  &=\frac{1}{n}\sum_{x=a}^b\frac{d^ne^{tx}}{dt^n}\nonumber\\
  &=\frac{1}{n}\sum_{x=a}^bx^ne^{tx}\\
  M_X^{\left(n\right)}\left(0\right)&=\frac{1}{n}\sum_{x=a}^bx^n
 \end{align}
 \subsection{特性関数}
 \begin{align}
  \phi_X\left(t\right)&=M_X\left(it\right)\nonumber\\
  &=\frac{1}{n}\sum_{x=a}^be^{itx}
 \end{align}
 \subsection{キュムラント母関数}
 \begin{align}
  K_X\left(t\right)&=\log M_X\left(t\right)\nonumber\\
  &=\log\frac{1}{n}\sum_{x=a}^be^{tx}
 \end{align}
 \subsection{平均}
...
よって成り立つ．
 ...
 \begin{align}
  ...
  \left(m+1\right)^2-m^2&=2m+1\nonumber\\
  \sum_{m=1}^n\left(\left(m+1\right)^2-m^2\right)&=\sum_{m=1}^n\left(2m+1\right)\nonumber\\
  \left(\sum_{m=1}^n\left(m+1\right)^2\right)-\left(\sum_{m=1}^nm^2\right)&=2\left(\sum_{m=1}^nm\right)+\left(\sum_{m=1}^n1\right)\nonumber\\
  \left(\sum_{m=2}^{n+1}m^2\right)-\left(\sum_{m=1}^nm^2\right)&=2\left(\sum_{m=1}^nm\right)+n\nonumber\\
  \left(n+1\right)^2-1&=\nonumber\\
  n^2+2n&=\nonumber\\
  2\left(\sum_{m=1}^nm\right)&=n^2+n\nonumber\\
  &=n\left(n+1\right)\nonumber\\
  \left(\sum_{m=1}^nm\right)&=\frac{1}{2}n\left(n+1\right)
 \end{align}
 \subsubsection{本証明}
 \begin{align}
  E\left(X\right)&=M_X^{\left(1\right)}\left(0\right)\nonumber\\
  &=\frac{1}{n}\sum_{n=a}^bn\nonumber\\
  &=\frac{1}{n}\left(\left(\sum_{n=1}^bn\right)-\left(\sum_{n=1}^{a-1}n\right)\right)\nonumber\\
  &=\frac{1}{n}\left(\frac{1}{2}b\left(b+1\right)-\frac{1}{2}\left(a-1\right)a\right)\nonumber\\
  &=\frac{1}{2n}\left(b\left(b+1\right)-\left(a-1\right)a\right)\nonumber\\
  &=\frac{1}{2n}\left(b^2+b-a^2+a\right)\nonumber\\
  &=\frac{1}{2\left(b-a+1\right)}\left(b-a+1\right)\left(a+b\right)\nonumber\\
  &=\frac{a+b}{2}
 \end{align}
 \subsection{分散}
 \subsubsection{自然数の2乗の和}
 \begin{align}
  \left(m+1\right)^3&=m^3+3m^2+3m+1\nonumber\\
  \left(m+1\right)^3-m^3&=3m^2+3m+1\nonumber\\
  \sum_{m=1}^n\left(\left(m+1\right)^3-m^3\right)&=\sum_{m=1}^n\left(3m^2+3m+1\right)\nonumber\\
  \left(\sum_{m=1}^n\left(m+1\right)^3\right)-\left(\sum_{m=1}^nm^3\right)&=\left(\sum_{m=1}^n3m^2\right)+\left(\sum_{m=1}^n3m\right)+\left(\sum_{m=1}^n1\right)\nonumber\\
  \left(\sum_{m=2}^{n+1}m^3\right)-\left(\sum_{m=1}^nm^3\right)&=3\left(\sum_{m=1}^nm^2\right)+3\left(\sum_{m=1}^nm\right)+n\nonumber\\
  \left(n+1\right)^3-1&=3\left(\sum_{m=1}^nm^2\right)+\frac{3}{2}n\left(n+1\right)+n\nonumber\\
  2\left(n+1\right)^3-2&=6\left(\sum_{m=1}^nm^2\right)+3n\left(n+1\right)+2n\nonumber\\
  2n^3+6n^2+6n&=6\left(\sum_{m=1}^nm^2\right)+3n^2+5n\nonumber\\
  6\left(\sum_{m=1}^nm^2\right)&=2n^3+3n^2+n\nonumber\\
  6\left(\sum_{m=1}^nm^2\right)&=n\left(n+1\right)\left(2n+1\right)\nonumber\\
  \sum_{m=1}^nm^2&=\frac{1}{6}n\left(n+1\right)\left(2n+1\right)
 \end{align}
 \subsection{標準偏差}
 \subsection{歪度}
 \subsection{尖度}
...
ベルヌーイ分布に従う互いに独立な$N$個の確率変数$X_1,\cdots,X_N$の和
 \begin{align}
  X&=\sum_{n=1}^NX_n
 \end{align}
を二項分布といい，その標本空間は$S=\{0,\cdots,N\}$である．
