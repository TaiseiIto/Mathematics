 ...
 \section{標準正規分布}
  \subsection{ポアソン分布から標準正規分布を導出}
   ...
   \subsubsection{自然数の$-2$乗の和の収束}
    ...
    よって単調増加である．
    よって収束する．
   \subsubsection{無乗積の収束}
    \begin{align}
     &s:\mathbb{N}\to\mathbb{R}\nonumber\\
     &p=\lim_{N\to\infty}\prod_{n=1}^Ns\left(n\right)
    \end{align}
    のとき，$p\in\mathbb{R}_{\ne 0}$を，無限積$p$は収束するという．
   \subsubsection{一様収束}
    以下が成り立つとき，関数列$f_n\left(x\right)$は関数$f\left(x\right)$に一様収束するという．
    \begin{align}
     \lim_{n\to\infty}\sup_x\left|f_n\left(x\right)-f\left(x\right)\right|=0
    \end{align}
   \subsubsection{ワイエルシュトラスのM判定法}
