...
互いに独立とは限らない離散確率変数$X$と連続確率変数$Y$があり，それらの確率密度関数が$p\left(x,y\right)$の形で与えられている．
この時，$X$単独の確率質量関数$p_X$は，
 \begin{align}
  p_X\left(x\right)&=\int_{-\infty}^\infty p\left(x,y\right)dy
 \end{align}
...
確率母関数$G_X$と確率質量関数$p$の間には，以下の関係が成り立つ．
 ...
 \begin{align}
  ...
  &=p\left(a\right)\left(\prod_{k=0}^{a-1}\left(a-k\right)\right)t^{a-a}+\sum_{x\in S|a<x}p\left(x\right)\left(\prod_{k=0}^{a-1}\left(x-k\right)\right)t^{x-a}\nonumber\\
