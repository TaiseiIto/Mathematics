...
を，連続確率変数$X$の分散という．
 \subsection{関数の適用}
微分可能で$f'\left(x\right)=0$の解が高々有限個である関数$f:S\to T$を連続確率変数$X=\left(S,P\right)$に適用した連続確率変数を$Y=\left(T,Q\right)$とし，その確率密度関数を$q$とすると，$f'\left(b\right)=0$を満たすような$b$の範囲内において，累積分布関数$Q$は，
 \begin{align}
  Q\left(b\right)&=\mathscr{P}\left(y<b\right)\nonumber\\
  &=\mathscr{P}\left(f\left(x\right)<b\right)\nonumber\\
...
連続確率変数$Y$の平均は
 \begin{align}
  E\left(Y\right)&=\int_{-\infty}^\infty yq\left(y\right)dy\nonumber\\
...
連続確率変数$Y$の分散は
 \begin{align}
  V\left(Y\right)&=\int_{-\infty}^\infty\left(y-E\left(Y\right)\right)^2q\left(y\right)dy\nonumber\\
  &=\int_{-\infty}^\infty\left(y-E\left(Y\right)\right)^2\sum_{x\in S|f\left(x\right)=y}\frac{p\left(x\right)}{f'\left(x\right)}dy\nonumber\\
  &=\int_{-\infty}^\infty\sum_{x\in S|f\left(x\right)=y}\left(y-E\left(Y\right)\right)^2\frac{p\left(x\right)}{f'\left(x\right)}dy\nonumber\\
  &=\int_{-\infty}^\infty\sum_{x\in S|f\left(x\right)=y}\left(f\left(x\right)-E\left(Y\right)\right)^2\frac{p\left(x\right)}{f'\left(x\right)}dy\nonumber\\
  &=\int_{-\infty}^\infty\left(f\left(x\right)-E\left(Y\right)\right)^2\frac{p\left(x\right)}{f'\left(x\right)}\frac{dy}{dx}dx\nonumber\\
  &=\int_{-\infty}^\infty\left(f\left(x\right)-E\left(Y\right)\right)^2\frac{p\left(x\right)}{f'\left(x\right)}f'\left(x\right)dx\nonumber\\
  &=\int_{-\infty}^\infty\left(f\left(x\right)-E\left(Y\right)\right)^2p\left(x\right)dx
 \end{align}
となる．
ここで，$f'\left(x\right)=0$となる$x$は高々有限個しかないため，この積分の結果には影響しない．
