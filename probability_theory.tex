...
それぞれをマクローリン展開すると，
 \begin{align}
  \left(\sum_{n=1}^\infty\frac{f^{\left(0\right)}\left(0\right)}{n!}x^n\right)^k=f\left(x\right)^k=g\left(x\right)=\sum_{m=1}^\infty\frac{g^{\left(m\right)}\left(0\right)}{m!}x^m
 \end{align}
...
ただし，Bellの多項式が部分集合同士の順番を考えていないのに対し，多項式の$k$乗の展開は，$k$個の順序付けられた多項式因子それぞれからひとつの項を因子として選択することによって展開後のひとつの項を計算することを考えると，ひとつの項を計算する際に選ばれた$k$個の因子の可能な全ての順番の数$k!$を掛ける必要があるから，
 \begin{align}
  ...
  &=k!\sum_{m=1}^\infty B_{m,k}\left(\frac{f^{\left(1\right)}\left(0\right)}{1!},\cdots,\frac{f^{\left(n\right)}\left(0\right)}{n!},\cdots,\frac{f^{\left(m-k+1\right)}\left(0\right)}{\left(m-k+1\right)!}\right)x^m
 \end{align}
である．
