...
とする離散確率変数$X$の分布を離散一様分布という．
 ...
 \begin{align}
  ...
  M_X^{\left(n\right)}\left(t\right)&=\frac{d^n}{dt^n}\frac{1}{n}\sum_{x=1}^ne^{tx}\nonumber\\
  &=\frac{1}{n}\sum_{x=1}^n\frac{d^ne^{tx}}{dt^n}\nonumber\\
  &=\frac{1}{n}\sum_{x=1}^nx^ne^{tx}\\
  M_X^{\left(n\right)}\left(0\right)&=\frac{1}{n}\sum_{x=1}^nx^n
 \end{align}
 \subsection{特性関数}
 \begin{align}
  \phi_X\left(t\right)&=M_X\left(it\right)\nonumber\\
